...
const validateRequestQuery = async (request, response) => {
  ...
  if (Object.keys(request.query).includes("accountType")){
		const accountTypeId = await knex(ACCOUNT_TYPES_TABLE).select("id")
			.where("name", request.query.accountType).first();
		if (!accountTypeId) // resolve missing account type
			return new Exception(404, `The selected account type "${request.query.accountType}" doesn't exist`).handle(
				request,
				response
			);
